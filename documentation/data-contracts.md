...
D12 counters persist across run restarts and distinguish planned logical calls from attempts and auxiliary API requests. Pilot: 15 answers + 20 judgments = 35 calls, at most five extra retries/40 attempts and 100 total API requests. Whole study: 311 planned calls + 25 retries = 336 attempts and 800 total API requests. Enforce phase reserves, 8,192 input tokens per attempt, 4,096 pilot/2,048 later output tokens, and aggregate ceilings of 2,752,512 input and 770,048 output tokens. Reserve capacity before dispatch, conservatively account for unavailable usage, and include thinking usage where applicable. See [D12](decisions.md#d12) for the phase allocation. A failed judgment remains unscored; shared models do not create independent quota pools.
